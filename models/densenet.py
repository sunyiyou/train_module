...
class DenseNet3(nn.Module):
    ...
    def forward(self, x, fc_params=(False, False)):
        feat = self.features(x)
        # if self.method is None or self.method != "sp":
        feat = F.avg_pool2d(feat, 8)
        feat = feat.view(-1, self.in_planes)
        # if self.method is None or self.method.find("ood") > -1:
        #     out = self.fc(feat, apply=fc_params[0])
        #     out_ood = F.sigmoid(self.fc_ood(feat, apply=fc_params[1]))
        #     # out_ood = self.fc_ood(feat)#, apply=fc_params[1])
        #     return out, out_ood
        # else:
        out = self.fc(feat)
            # out = self.fc(F.dropout(feat, 0.1))
            # out = self.fc(sum([F.dropout(feat, 0.1) for _ in range(15)]) / 15)
        return out
